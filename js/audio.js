...
(function(){
  ...
  function startDistantMonstersAmbience(){if(!pageActive||document.hidden||endingActive)return;stopDungeonAmbience();ensureDistant().play().catch(()=>{});if(musicVolume>0&&!radioPaused)ensureRock()?.play().catch(()=>{});}
  ...
  function loadRadioTrack(index,play=true){if(!availableRadioTracks.length)return;rockIndex=(index+availableRadioTracks.length)%availableRadioTracks.length;const track=currentRadioTrack();if(!track)return;const player=ensureRock();player.pause();player.src=track.file;player.currentTime=0;player.volume=musicVolume;rememberRadioTrack();if(play&&(dungeonWanted||startScreenVisible())&&musicVolume>0&&!radioPaused&&pageActive&&!document.hidden&&!endingActive)player.play().catch(()=>{});refreshRadioPanel();}
  ...
  function radioPrevious(){loadRadioTrack(rockIndex-1,true);}function radioNext(){loadRadioTrack(rockIndex+1,true);}function radioTogglePlay(){radioPaused=!radioPaused;if(radioPaused)rockAudio?.pause();else if((dungeonWanted||startScreenVisible())&&musicVolume>0&&!endingActive)ensureRock()?.play().catch(()=>{});refreshRadioPanel();}function radioSelect(index){radioPaused=false;loadRadioTrack(Number(index),true);}function radioToggleList(){radioListOpen=!radioListOpen;refreshRadioPanel();}
  ...
  function refreshDesktopRadio(){const panel=document.getElementById('desktopDungeonRadio');if(!panel)return;panel.innerHTML=desktopRadioHTML();panel.hidden=!(startScreenVisible()&&window.matchMedia('(min-width:901px)').matches&&!endingActive);}
